...
import { apiListLogs, apiUpdatePost, apiQueuePublish, apiDeletePost, apiDeleteMedia } from "@/lib/api";
...
export default function PublishingLogsView() {
  ...
  const deletePost = async (id: string) => {
    try {
      ...
      setLogs((prev) => prev.filter((p) => p.id.toString() !== id.toString()));
    } catch (e) {
  ...
  const filteredLogs = useMemo(() => {
    return logs
      .map(post => {
        const platformName = normalizePlatformName(post.platform);
    ...
    <div className="flex-1 overflow-y-auto p-6 lg:p-10 custom-scrollbar flex flex-col min-w-0 bg-transparent">
      ...
      <div className="space-y-4 min-w-0">
        {filteredLogs.map((log) => {
          ...
          const statusMeta = statusConfig[log.status as PostStatus] || statusConfig.Draft;
            ...
            >
              ...
              <div className="flex items-start gap-4 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {/* Media Thumbnails (S3 URLs) */}
                  {Array.isArray(log.media_urls) && log.media_urls.length > 0 && (
                    <div className="flex items-center gap-2 mt-3">
                      {log.media_urls.slice(0, 3).map((url: string, idx: number) => (
                        <div key={idx} className="relative w-14 h-14 rounded-lg overflow-hidden border border-white/10 bg-black/30 shrink-0">
                      ...
                      {log.media_urls.length > 3 && (
                        <div className="w-14 h-14 rounded-lg border border-white/10 bg-white/5 flex items-center justify-center text-xs font-bold text-white/50 shrink-0">
                          +{log.media_urls.length - 3}
                        </div>
                      )}
                    </div>
