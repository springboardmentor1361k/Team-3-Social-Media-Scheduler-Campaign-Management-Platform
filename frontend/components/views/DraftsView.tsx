...
export default function DraftsView() {
  ...
  }, [posts, platformFilter, typeFilter, categoryFilter, searchQuery]);
  ...
  const handleBulkDelete = () => {
    ...
    setSelectedIds([]);
    showToast(`Deleted ${count} draft(s)`);
  };
  ...
  const handleConfirmBulkSchedule = () => {
    ...
    const count = selectedIds.length;
    bulkScheduleDrafts(selectedIds, combined.toISOString());
    setSelectedIds([]);
  ...
  const handleConfirmSchedule = () => {
    ...
    scheduleDraft(scheduleModalPost.id, combinedDate.toISOString());
  ...
  const handleOpenEdit = (post: SocialPost) => {
    ...
    setDraftCampaign(post.campaign);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 rounded-2xl bg-gradient-to-r from-violet-900/40 via-indigo-900/30 to-purple-900/40 border border-violet-500/20 backdrop-blur-xl relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-black text-white tracking-tight">Draft Management</h1>
          </div>
          <p className="text-sm text-white/50">
            Create, categorize, edit, and schedule unpublished post ideas.
          </p>
        </div>
