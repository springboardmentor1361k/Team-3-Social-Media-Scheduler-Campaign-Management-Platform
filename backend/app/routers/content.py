...
async def update_post(
    ...
    content_fields = {"content", "media_urls", "content_type", "platform", "draft_metadata"}
    if data.model_fields_set & content_fields:
        await mongo_content_service.save_draft(
            ...
            extra_metadata=data.draft_metadata,
        )
...
def list_publishing_logs(
    ...
    # Build a lookup of social_account_id → account_name
    account_ids = [p.social_account_id for p in posts if p.social_account_id]
    accounts = {}
    if account_ids:
        rows = db.query(SocialAccount).filter(SocialAccount.id.in_(account_ids)).all()
        accounts = {a.id: a.account_name for a in rows}
    ...
    for p in posts:
        logs.append(
            PublishLogOut(
                ...
                account_name=accounts.get(p.social_account_id),
                platform=p.platform,
