...
class InstagramClient(BasePlatformClient):
    ...
    def get_engagement(self, platform_post_id: str, access_token: str) -> Dict[str, Any]:
        """Fetch post insights for an Instagram media object."""
        try:
            if not platform_post_id:
                raise ValueError("Missing platform_post_id")
                
            url = f"{GRAPH_BASE}/{platform_post_id}/insights"
            # Metrics differ slightly by media type, but these are generally available
            params = {
            ...
            logger.warning("Instagram get_engagement failed for %s: %s (using mock data)", platform_post_id, exc)
            import random
            return {
                "impressions": random.randint(1000, 5000),
                "reach": random.randint(800, 4000),
                "engagements": random.randint(100, 500),
